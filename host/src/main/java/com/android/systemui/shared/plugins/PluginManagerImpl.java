...
/**
 * 插件管理器实现，负责监听包变化并协调各 PluginActionManager 的加载/卸载。
 *
 * @see Plugin
 */
public class PluginManagerImpl extends BroadcastReceiver implements PluginManager {
    ...
    private boolean clearClassLoader(String pkg) {
        if (mPluginInstanceFactory.clearClassLoader(pkg) != null) {
            return true;
        }
        return false;
    }
